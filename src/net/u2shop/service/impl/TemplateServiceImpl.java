...
import net.u2shop.Template;
import net.u2shop.Template.Type;
import net.u2shop.service.TemplateService;
...
public class TemplateServiceImpl implements TemplateService, ServletContextAware {
	...
	public List<Template> getAll() {
		try {
			File u2shopXmlFile = new ClassPathResource(CommonAttributes.U2SHOP_XML_PATH).getFile();
			Document document = new SAXReader().read(u2shopXmlFile);
			List<Template> templates = new ArrayList<Template>();
			List<Element> elements = document.selectNodes("/u2shop/template");
	...
	public List<Template> getList(Type type) {
		if (type != null) {
			try {
				File shopxxXmlFile = new ClassPathResource(CommonAttributes.U2SHOP_XML_PATH).getFile();
				Document document = new SAXReader().read(shopxxXmlFile);
				List<Template> templates = new ArrayList<Template>();
				List<Element> elements = document.selectNodes("/u2shop/template[@type='" + type + "']");
				for (Element element : elements) {
					Template template = getTemplate(element);
					templates.add(template);
				}
	...
	public Template get(String id) {
		try {
			File u2shopXmlFile = new ClassPathResource(CommonAttributes.U2SHOP_XML_PATH).getFile();
			Document document = new SAXReader().read(u2shopXmlFile);
			Element element = (Element) document.selectSingleNode("/u2shop/template[@id='" + id + "']");
			Template template = getTemplate(element);
			return template;
		} catch (Exception e) {
			e.printStackTrace();
