...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.jhtml">首页</a> &raquo; 角色列表 <span>(共${page.total}条记录)</span>
	</div>
	<form id="listForm" action="list.jhtml" method="get">
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort" name="isSystem">收费内置</a>
				</th>
				<th>
					<span>描述</span>
				</th>
			...
			[#list page.content as role]
				<tr>
					<td>
						<input type="checkbox" name="ids"[#if role.isSystem] title="不许删" disabled="disabled"[#else] value="${role.id}"[/#if] />
					</td>
					<td>
						${role.name}
					</td>
					<td>
						[#if role.isSystem == "true"]是[#else]否[/#if]
					</td>
					<td>
						[#if role.description??]
